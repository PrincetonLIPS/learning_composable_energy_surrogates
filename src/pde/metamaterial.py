...
class Metamaterial(PDE):
    def _build_mesh(self):
        ...
        args = self.args
        (L0, porosity, c1, c2, resolution, n_cells, min_feature_size,
         pore_radial_resolution) = (args.L0, args.porosity, args.c1, args.c2,
                                    args.metamaterial_mesh_size, args.n_cells,
    ...
    def _build_function_space(self):
        ...
        class Exterior(fa.SubDomain):
            def inside(self, x, on_boundary):
                return on_boundary and (fa.near(x[1], L0 * n_cells) or fa.near(
                    x[0], L0 * n_cells) or fa.near(x[0], 0)
                                        or fa.near(x[1], 0))
