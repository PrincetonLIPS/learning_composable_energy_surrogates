...
class Collector(object):
    def __init__(self, args):
        ...
        self.bc, _, _, self.constraint_mask = make_bc(
            args, self.fsm)
    ...
    def increment_factor(self):
        self.factor = np.min([1.,
                              self.factor +
                              self.stepsize * (1 + random.random() - 0.5)])
...
class PolicyCollector(Collector):
    def __init__(self, args, surrogate):
        ...
        _, self.traj_u, _ = surrogate.solve(
            params, self.bc, self.constraint_mask, force_data,
            return_intermediate=True)

        if args.weight_space_trajectory:
            deltas = [
                torch.norm(
                    self.traj_u[i] - self.traj_u[i-1]).data.cpu().numpy()
