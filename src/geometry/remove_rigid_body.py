...
class RigidRemover(object):
    def __init__(self, fsm):
        ...
        self.ref_cpu = remove_translation(
            fsm.ring_coords.detach().clone().view(1, -1, 2)).view(-1, 2)
        if fsm.cuda:
    ...
    def __call__(self, inputs):
        ret = remove_rotation(
            ...
            self.ref.view(1, -1, 2), self.ref) - self.ref.view(1, -1, 2)
        # pdb.set_trace()
...
def batched_2x2_svd(A):
    ...
    return torch.Tensor(U).to(A.device), torch.Tensor(s).to(
        A.device), torch.Tensor(V).to(
            A.device)  # Maybe replace with custom later
...
def remove_rotation(x, ref):
    ...
    return torch.matmul(R.transpose(-2, -1), x.permute(0, 2,
                                                       1)).permute(0, 2, 1)
...
if __name__ == '__main__':
    ...
    R = torch.Tensor(
        np.array([[np.cos(thetas), -np.sin(thetas)],
                  [np.sin(thetas), np.cos(thetas)]])).permute(2, 0, 1)
    trans = torch.Tensor(np.random.randn(256, 1, 2))
    x = torch.matmul(R,
                     ref.unsqueeze(0).permute(0, 2, 1)).permute(0, 2,
                                                                1) + trans
