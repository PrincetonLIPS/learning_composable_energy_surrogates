...
if __name__ == '__main__':
    ...
    try:
        ...
        for step in range(1, args.max_train_steps):
            ...
            if args.visualize_every > 0 and (step -
                                             1) % args.visualize_every == 0:
                trainer.visualize(step - 1, trainer.train_plot_data,
                                  'Training')
                trainer.visualize(step - 1, trainer.val_plot_data,
                                  'Validation')
            with open(os.path.join(out_dir, 'losses.txt'), 'a') as lossfile:
                if evaluator is not None and evaluator.normalizer > 0:
                    ...
                    deploy_loss = None
                lossfile.write('{}: tloss: {}, vloss: {}, dloss: {}\n'.format(
