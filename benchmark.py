...
ckpt = torch.load('/efs_nmor/results/bV_10_hmc_macro/' + load_ckpt_path + '/ckpt_last.pt', map_location=torch.device('cpu'))
...
Xi1s = [0., -0.1, 0.1, -0.2, -0.16, -0.23, -0.4] 
Xi2s = [0., 0.1, -0.1, 0.07, 0.21, -0.18, -0.06]
...
for Xi1, Xi2 in zip(Xi1s, Xi2s):
    ...
    with Timer() as t:
        surr_soln, traj_u, traj_f, traj_g = cem.solve(params, boundary_data,
                          cem_constraint_mask, force_data,
    ...
    torch.save({'Xi': (Xi1, Xi2), 'surr_time': surr_time, 'surr_numel': surr_numel,
                'surr_E': surr_E, 'surr_coords': surr_coords, 
                'mesh_sizes': MESH_SIZES, 'pore_resolutions': PORE_RES,
                'fem_times': fem_times, 'fem_numels': fem_numels,
                'fem_coords': fem_coords, 'fem_Es': fem_Es, 'fem_fams': fem_fams},
